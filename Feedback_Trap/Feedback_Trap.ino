...
const int targetTicks = 5000;  // Target distance in encoder ticks
...
void loop() {
  ...
  // Set motor speed
  if(forward){
  setMotorForwardSpeed(speed);
  }else{
  setMotorBackwardSpeed(speed);
...
int calculateSpeed(long currentTicks, long targetTicks, int maxSpeed, float accelRate, float decelRate) {
  ...
  if(currentTicks < 60 || remainingTicks < 0){
    currentSpeed = 15;
  }
  // if (remainingTicks < 0) {
  //   remainingTicks = 0;
...
void setMotorForwardSpeed(int speed) {
...
void setMotorBackwardSpeed(int speed) {
