...
#define pwm3 15
#define pwm4 25
 int target = 5000; // Example target calculation, replace with your own logic
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(12), ai0_1, RISING); // Encoder 1 pin A interrupt
  attachInterrupt(digitalPinToInterrupt(21), ai1_1, RISING); 
  pinMode(pwm1, OUTPUT);
  pinMode(pwm2, OUTPUT);
  pinMode(pwm3, OUTPUT);
  pinMode(pwm4, OUTPUT);
    ...
    if (ps5.Square()) {
      ...
      if (squarePressed) {
        ...
        if (motorDir == 1) {
          ...
          analogWrite(pwm3, pwr);
          analogWrite(pwm4, pwr);
